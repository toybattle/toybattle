...
def gameSolo(screen, clock):
    ...
    pygame.display.set_caption(f"Toy Battle - Partie Solo)")
    ...
    try:
        ...
        map_index = random.randint(0, len(datamap) - 1)  # Choix aléatoire de la map
    ...
    def execute_ability(card, active_units, hand, deck):
        ...
        if "Piocher 2 cartes" in desc:
            ...
            enemy_units = [u for u in active_units if any(t.get("player") == "player2" for t in datamap[map_name]["tiles"] if t["id"] == u.tile_id)]
            if enemy_units:
